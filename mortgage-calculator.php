...
<head>
    ...
    <meta name="keywords" content="home loan emi calculator,house loan emi calculator,loan emi calculator home loan,home loan monthly emi calculator,home loan calculator emi calculator,home loan and emi calculator,home loan emi cal,loan calculator,lending calculator,home loan calculator
">
...
<body>
        ...
        <div class="container calculation_top">
            <div class="d-flex flex-column">
                <div class="d-flex  align-items-center common_links_service">
                    ...
                    <p>></p>
                    <a class="blod_text" href="#"> Mortgage Calculator</a>
                </div>
                <div class="d-flex align-items-center calculator_head">
                    <h1 style="color:  rgba(66, 66, 65, 1);">Mortgage Calculator</h1>
                </div>
            </div>
    ...
    <div class="container">
        <div class="row calculator-container mortage-container">
            <div class="col-lg-6 cal_left_div">
                <div class="input-section">
                    <div class="input-group">
                        <label for="loanAmount">Loan Amount (₹)</label>
                      <div class="range_slide">
                        <input type="range" id="loanAmount" min="0" max="50000000" step="100000" value="0">
                      </div>
                      <div class="range_input">
                        <input type="number" id="loanAmountInput" min="0" max="50000000" step="100000" value="0">
                      </div>
                    </div>
                    <div class="input-group">
                        <label for="loanTerm">Loan Term (in years)</label>
                      <div class="range_slide">
                        <input type="range" id="loanTerm" min="1" max="30" step="1" value="0">
                      </div>
                      <div class="range_input">
                        <input type="number" id="loanTermInput" min="1" max="30" step="1" value="0">
                      </div>
                    </div>
                    ...
                    <div class="input-group">
                      ...
                      <div class="range_slide">
                        <input type="range" id="interestRate" min="0.1" max="15" step="0.1" value="0">
                      </div>
                      <div class="range_input">
                        <input type="number" id="interestRateInput" min="0.1" max="15" step="0.1" value="0">
                      </div>
                    </div>
                    ...
                    <div class="input-group">
                      ...
                      <div class="range_slide">
                        <input type="range" id="insurance" min="0" max="10000" step="100" value="0">
                      </div>
                      <div class="range_input">
                        <input type="number" id="insuranceInput" min="0" max="10000" step="100" value="0">
                      </div>
                    </div>

                    <div class="input-group">
                        <label for="propertyTaxes">Property Taxes (₹ per month)</label>
                      <div class="range_slide">
                        <input type="range" id="propertyTaxes" min="0" max="10000" step="100" value="0">
                      </div>
                      <div class="range_input">
                        <input type="number" id="propertyTaxesInput" min="0" max="10000" step="100" value="0">
                      </div>
                    </div>
    ...
    const loanAmountBox = document.getElementById("loanAmountInput");
    const loanTermInput = document.getElementById("loanTerm");
    const loanTermBox = document.getElementById("loanTermInput");
    const interestRateInput = document.getElementById("interestRate");
    const interestRateBox = document.getElementById("interestRateInput");
    const insuranceInput = document.getElementById("insurance");
    const insuranceBox = document.getElementById("insuranceInput");
    const propertyTaxesInput = document.getElementById("propertyTaxes");
    const propertyTaxesBox = document.getElementById("propertyTaxesInput");
  
    function calculateMortgage() {

     const loanAmount = parseFloat(loanAmountInput.value) || 0;  // Default to 0 if not a valid number
  const loanTerm = parseInt(loanTermInput.value) || 0;  // Default to 0 if not a valid number
  const interestRate = parseFloat(interestRateInput.value) || 0;  // Default to 0 if not a valid number
  const insurance = parseFloat(insuranceInput.value) || 0;  // Default to 0 if not a valid number
  const propertyTaxes = parseFloat(propertyTaxesInput.value) || 0;  // Default to 0 if not a valid number
  
      // Sync inputs
      loanAmountBox.value = loanAmount;
      loanTermBox.value = loanTerm;
      interestRateBox.value = interestRate;
      insuranceBox.value = insurance;
      propertyTaxesBox.value = propertyTaxes;
  
      // Mortgage Calculation (Using the formula for monthly payments)
    ...
    loanAmountInput.addEventListener("input", () => {
      loanAmountBox.value = loanAmountInput.value;
      calculateMortgage();
    });
    loanAmountBox.addEventListener("input", () => {
      loanAmountInput.value = loanAmountBox.value;
      calculateMortgage();
    });
  
    loanTermInput.addEventListener("input", () => {
      loanTermBox.value = loanTermInput.value;
      calculateMortgage();
    });
    loanTermBox.addEventListener("input", () => {
      loanTermInput.value = loanTermBox.value;
      calculateMortgage();
    });
  
    interestRateInput.addEventListener("input", () => {
      interestRateBox.value = interestRateInput.value;
      calculateMortgage();
    });
    interestRateBox.addEventListener("input", () => {
      interestRateInput.value = interestRateBox.value;
      calculateMortgage();
    });
  
    insuranceInput.addEventListener("input", () => {
      insuranceBox.value = insuranceInput.value;
      calculateMortgage();
    });
    insuranceBox.addEventListener("input", () => {
      insuranceInput.value = insuranceBox.value;
      calculateMortgage();
    });
  
    propertyTaxesInput.addEventListener("input", () => {
      propertyTaxesBox.value = propertyTaxesInput.value;
      calculateMortgage();
    });
    propertyTaxesBox.addEventListener("input", () => {
      propertyTaxesInput.value = propertyTaxesBox.value;
      calculateMortgage();
    });
