...
<head>
    ...
    <meta name="keywords" content="sip calculator,sip plan calculator,systematic investment plan calculator,sip investment plan calculator,sip cal,systematic investment calculator,swp calculator,systematic withdrawal plan calculator,mutual fund calculator,muchhal fund calculator
">
    <meta name="description" content="Calculate your SIP returns effortlessly with our mutual fund SIP calculator. Plan and optimize your investments for financial success. 
">
...
<body>
        ...
        <div class="container calculation_top">
            <div class="d-flex flex-column">
                <div class="d-flex  align-items-center common_links_service">
                    ...
                    <p>></p>
                    <a class="blod_text" href="#">SIP Calculator</a>
                </div>
                <div class="d-flex align-items-center calculator_head">
                    <h1 style="color:  rgba(66, 66, 65, 1);">SIP Calculator</h1>
                </div>
            </div>
        ...
        <div class="row calculator-container">
            <div class="col-lg-6 cal_left_div">
                <div class="input-section">
                    <div class="tabs">
                        <button class="calculator-tab active" data-type="sip" onclick="setCalculator('sip')">SIP</button>
                        <button class="calculator-tab" data-type="lumpsum" onclick="setCalculator('lumpsum')">Lumpsum</button>
                    </div>

                    <div class="input-group">
                        <label for="investmentAmount" id="investmentLabel">Monthly investment</label>
                      <div class="range_slide">
                        <input type="range" id="investmentAmountRange" min="100" max="100000" step="100" value="0">
                      </div>
                      <div class="range_input">
                        <input type="number" id="investmentAmount" min="100" max="100000" step="100" value="0">
                      </div>
                    </div>
                    <div class="input-group">
                        <label for="rate">Expected Return Rate (p.a):</label>
                      <div class="range_slide">
                        <input type="range" id="rateRange" min="1" max="20" step="0.1" value="0">
                      </div>
                      <div class="range_input">
                        <input type="number" id="rate" min="1" max="20" step="0.1" value="0">
                      </div>
                    </div>
                    <div class="input-group">
                        <label for="timePeriod">Time Period (Years):</label>
                      <div class="range_slide">
                        <input type="range" id="timePeriodRange" min="1" max="30" step="1" value="1">
                      </div>
                      <div class="range_input">
                        <input type="number" id="timePeriod" min="1" max="30" step="1" value="1">
                      </div>
                    </div>
    ...
    <script>
    let calculatorType = 'sip'; // Default to SIP calculator
    let sharedValues = {
        investmentAmount: 0,
        rate: 0,
        timePeriod: 1
    ...
    function setCalculator(type) {
        ...
        // Update the label
        document.getElementById('investmentLabel').innerText =
            type === 'sip' ? 'Monthly Investment:' : 'Total Investment:';

        // Add active class to the selected tab and remove from others
    ...
    function updateFieldValues() {
        sharedValues.investmentAmount = parseFloat(document.getElementById('investmentAmount').value) || 0;
        sharedValues.rate = parseFloat(document.getElementById('rate').value) || 0;
        sharedValues.timePeriod = parseFloat(document.getElementById('timePeriod').value) || 1;
        calculate();
    }

    function calculate() {
        const { investmentAmount, rate, timePeriod } = sharedValues;
        const n = timePeriod * 12; // Monthly periods for SIP
        const i = rate / 12 / 100; // Monthly interest rate
        let totalInvested = calculatorType === 'sip' ? investmentAmount * n : investmentAmount;
        let totalValue = 0;
        ...
        if (calculatorType === 'sip') {
            ...
            totalValue = investmentAmount * Math.pow(1 + rate / 100, timePeriod);
            estimatedReturns = totalValue - totalInvested;
        }

        document.getElementById('investedAmount').innerText = `₹ ${totalInvested.toLocaleString()}`;
        document.getElementById('estimatedReturns').innerText = `₹ ${estimatedReturns.toLocaleString()}`;
    ...
    document.getElementById('investmentAmount').addEventListener('input', function() {
        document.getElementById('investmentAmountRange').value = this.value;
        updateFieldValues();
    });
    ...
    document.getElementById('timePeriod').addEventListener('input', function() {
        document.getElementById('timePeriodRange').value = this.value;
        updateFieldValues();
    });
