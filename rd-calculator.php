...
<head>
    ...
    <meta name="keywords" content="rd calculator,recurring deposit calculator,rd account calculator,rd deposit calculator,recurring calculator,rd recurring deposit calculator,recurring account calculator,recurring deposit account calculator,recurring deposit cal,post office rd calculator
">
    <meta name="description" content="RD Calculator - Calculate interest rate &amp; maturity value of your deposit Recurring deposit calculator. Visit us to use our online rd calculator now
">
    ...
    <link href="https://unpkg.com/aos@2.3.1/dist/aos.css" rel="stylesheet">
    <title>RD Calculator : Recurring Deposit Interest Rate Calculator Online
</title>
        ...
        <div class="container calculation_top">
            <div class="d-flex flex-column">
                <div class="d-flex  align-items-center common_links_service">
                    ...
                    <p>></p>
                    <a class="blod_text" href="#"> RD Calculator</a>
                </div>
                <div class="d-flex align-items-center calculator_head">
                    <h1 style="color:  rgba(66, 66, 65, 1);">RD Calculator</h1>
                </div>
            </div>
                ...
                <div class="input-section">
                    <div class="input-group">
                      ...
                      <div class="range_slide">
                        <input type="range" id="monthlyInvestment" min="500" max="1000000" step="500" value="0">
                      </div>
                      <div class="range_input">
                        <input type="number" id="monthlyInvestmentInput" min="500" max="1000000" step="500" value="0">
                      </div>
                    </div>
                    <div class="input-group">
                        <label for="rateOfInterest">Rate of Interest (p.a.)</label>
                      <div class="range_slide">
                        <input type="range" id="rateOfInterest" min="1" max="12" step="0.1" value="0">
                      </div>
                      <div class="range_input">
                        <input type="number" id="rateOfInterestInput" min="1" max="12" step="0.1" value="0">
                      </div>
                    </div>
                    <div class="input-group">
                        <label for="timePeriod" class="timePeriod">Time Period  
                            <!-- Checkbox styled as a switcher -->
                             <div class="slide_switcger">
                                <span class="slider_main">
                            <input type="checkbox" id="toggleTimePeriodBtn" class="switcher-checkbox">
                            <span class="slider"></span>
                        </span>
                            <b>Calculating for:-</b><span class="switcher-label">Years</span>
                        </div>
                        </label>
                        <div class="range_slide">
                            <input type="range" id="timePeriod" min="1" max="120" step="1" value="1">
                        </div>
                        <div class="range_input">
                            <input type="number" id="timePeriodInput" min="1" max="120" value="1">
                        </div>
                    </div>
                  </div>
            </div>
            <div class="col-lg-6 cal_left_div">
                <div class="result-section">
                    ...
                    <div class="two_bomtm">
                      <div class="fadee">
                        <h2>Est. returns</h2>
                        <span id="estimatedReturns">₹ 0</span>
                      </div>
...
const monthlyInvestmentBox = document.getElementById("monthlyInvestmentInput");
const timePeriodRange = document.getElementById("timePeriod");
...
const rateOfInterestBox = document.getElementById("rateOfInterestInput");
const investedAmountDisplay = document.getElementById("investedAmount");
const estimatedReturnsDisplay = document.getElementById("estimatedReturns");
const totalValueDisplay = document.getElementById("totalValue");
const toggleTimePeriodBtn = document.getElementById("toggleTimePeriodBtn");
const switcherLabel = document.querySelector(".switcher-label");

let isMonthsSelected = false; // Start with years selected by default

// Set initial state for years
function initialize() {
    // Set initial state to "Years"
    switcherLabel.innerText = "Years";
    timePeriodRange.max = timePeriodInput.max = 10; // Max for years
    timePeriodRange.min = timePeriodInput.min = 1;  // Min for years
    timePeriodInput.value = 1; // Set initial value for years
    timePeriodRange.value = 1; // Set range value to 1 as well

    // Calculate RD on load
    calculateRD();
}

// Toggle between months and years based on checkbox state
toggleTimePeriodBtn.addEventListener("change", () => {
    isMonthsSelected = toggleTimePeriodBtn.checked;

    // Update the label text based on selection
    switcherLabel.innerText = isMonthsSelected ? "Months" : "Years";

    const maxTime = isMonthsSelected ? 120 : 10;
    const minTime = isMonthsSelected ? 1 : 1;
    timePeriodRange.max = maxTime;
    timePeriodRange.min = minTime;
    timePeriodInput.max = maxTime;
    timePeriodInput.min = minTime;

    // Reset values to 1 when toggling between months and years
    timePeriodInput.value = 1;
    timePeriodRange.value = 1;

    // Recalculate the RD when toggling
    calculateRD();
});

// RD calculation logic
function calculateRD() {
    const principal = parseFloat(monthlyInvestmentInput.value);
    const rate = parseFloat(rateOfInterestInput.value);
    let timePeriod = parseInt(timePeriodInput.value);

    // Convert months to years if needed
    if (isMonthsSelected) timePeriod /= 12;

    const totalInvested = principal * timePeriod * 12;
    const monthlyRate = rate / 100 / 12;
    let totalMaturityValue = 0;

    // Compound interest calculation for monthly deposits
    for (let month = 1; month <= timePeriod * 12; month++) {
        totalMaturityValue += principal * Math.pow(1 + monthlyRate, month);
    }
    const estimatedReturns = totalMaturityValue - totalInvested;

    // Display the results with ₹ symbol
    investedAmountDisplay.innerText = "₹ " + totalInvested.toLocaleString();
    estimatedReturnsDisplay.innerText = "₹ " + estimatedReturns.toLocaleString();
    totalValueDisplay.innerText = "₹ " + totalMaturityValue.toLocaleString();
}

// Event listeners to update values on input change
monthlyInvestmentInput.addEventListener("input", () => {
    monthlyInvestmentBox.value = monthlyInvestmentInput.value;
    calculateRD();
});

monthlyInvestmentBox.addEventListener("input", () => {
    monthlyInvestmentInput.value = monthlyInvestmentBox.value;
    calculateRD();
});

timePeriodRange.addEventListener("input", () => {
    timePeriodInput.value = timePeriodRange.value;
    calculateRD();
});

timePeriodInput.addEventListener("input", () => {
    timePeriodRange.value = timePeriodInput.value;
    calculateRD();
});

rateOfInterestInput.addEventListener("input", () => {
    rateOfInterestBox.value = rateOfInterestInput.value;
    calculateRD();
});

rateOfInterestBox.addEventListener("input", () => {
    rateOfInterestInput.value = rateOfInterestBox.value;
    calculateRD();
});
...
// Initialize all range sliders and number inputs
const rangeSliders = document.querySelectorAll('.calculator-container .range_slide input[type="range"]');
const numberInputs = document.querySelectorAll('.calculator-container .range_input input[type="number"]');
const toggleButtons = document.querySelectorAll('#toggleTimePeriodBtn');  // For the button toggle

// Add event listeners to range sliders
rangeSliders.forEach(slider => {
    const numberInput = slider.closest('.input-group').querySelector('.range_input input[type="number"]');

    // Update number input and background size when range slider changes
...
numberInputs.forEach(numberInput => {
    const rangeInput = numberInput.closest('.input-group').querySelector('.range_slide input[type="range"]');

    // Update range slider and background size when number input changes
...
// Button toggle functionality (for checkbox)
toggleButtons.forEach(button => {
    button.addEventListener('change', function () { // Use 'change' event for checkbox
        const timePeriodSlider = document.querySelector('#timePeriod');
        const timePeriodInput = document.querySelector('#timePeriodInput');

        // Reset the background fill and input values when the toggle is clicked
        timePeriodSlider.style.backgroundSize = '0% 100%'; // Reset the fill
        timePeriodSlider.value = 1; // Reset the slider value
        timePeriodInput.value = 1; // Reset the number input value

        // Toggle between Years and Months based on checkbox
        if (button.checked) {
            // Change to Months
            timePeriodSlider.max = 12 * 10; // Example: 10 years = 120 months
            timePeriodInput.max = 12 * 10;
        } else {
            // Change to Years
            timePeriodSlider.max = 120; // Example: 120 months = 10 years
            timePeriodInput.max = 120;
        }

        // Update the background size (which will be 0% since the value is reset)
        updateSliderBackground(timePeriodSlider);
    });
});
